...
import { ObsidianPropertiesSchema, PropertyUpdateSchema } from './types.js';
...
export class PropertyManager {
    ...
    parseProperties(content) {
        try {
            ...
            if (!validationResult.success) {
                ...
                return {
                    properties: rawProperties, // Cast, knowing it's invalid
            ...
            if (validatedProperties.tags && Array.isArray(validatedProperties.tags)) {
                validatedProperties.tags = validatedProperties.tags.map((tag) => typeof tag === 'string' && tag.startsWith('#') ? tag.substring(1) : tag);
            }
    ...
    generateProperties(properties) {
        try {
            ...
            if (propsToGenerate.tags && Array.isArray(propsToGenerate.tags)) {
                propsToGenerate.tags = propsToGenerate.tags.map((tag) => typeof tag === 'string' && !tag.startsWith('#') ? `#${tag}` : tag);
            }
            // Remove undefined values explicitly (shouldn't be necessary with JSON.stringify/parse but safe)
            const cleanProperties = Object.fromEntries(Object.entries(propsToGenerate).filter(([_, v]) => v !== undefined));
    ...
    mergeProperties(existing, updates, replace = false) {
        ...
        for (const [key, value] of Object.entries(updates)) {
            ...
            if (value === undefined || key === 'modified')
                continue;
            ...
            if (Array.isArray(value) && Array.isArray(currentValue)) {
                merged[key] = replace ?
                    value : // Replace array
                    [...new Set([...currentValue, ...value])]; // Merge unique elements
            }
